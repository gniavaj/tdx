...
{
	tdxLog("code:" + charPointToString(Code));
	tdxLog(unsignedShortToString(time1.year) + unsignedCharToString(time1.month) + unsignedCharToString(time1.day));
	tdxLog(unsignedShortToString(time2.year) + unsignedCharToString(time2.month) + unsignedCharToString(time2.day));
	
	BOOL nRet = FALSE;
	NTime tmpTime={0};

	//窥视数据个数
	long datanum = g_pFuncCallBack(Code,nSetCode,DataType,NULL,-1,tmpTime,tmpTime,nTQ,0);
	tdxLog("datanum:" + longToString(datanum));
	...
	tdxLog("readnum:" + longToString(readnum));
